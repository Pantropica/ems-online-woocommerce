...
function woocommerce_ginger_init()
{
	...
	load_plugin_textdomain("emspay", false, basename(dirname(__FILE__)) . '/languages');
	...
	function ginger_refund_an_order($refund_id, $args)
	{
		...
		if ($gingerOrder['status'] !== 'completed') {
			throw new Exception(__('Only completed orders can be refunded', "emspay"));
		}
		...
		if (in_array($order->get_payment_method(), WC_Ginger_Helper::GATEWAYS_SUPPORT_CAPTURING)) {
			if (!in_array('has-captures', $gingerOrder['flags'])) {
				throw new Exception(__('Refunds only possible when captured', "emspay"));
			};
		...
		if ($gingerRefundOrder['status'] !== 'completed') {
			if (current($gingerRefundOrder['transactions'])['customer_message']) {
				...
				$notice = __('Refund order is not completed: %s', "emspay");
				throw new Exception(sprintf(
					$notice,
					current($gingerRefundOrder['transactions'])['customer_message']
				));
			} else {
				throw new Exception(__('Refund order is not completed', "emspay"));
			}
		}
	}
	...
	function ginger_add_refund_description($order)
	{
		...
		{
			echo "<p style='color: red; ' class='description'>" . esc_html__("Please beware that for bank transactions the refunds will process directly to the gateway!", "emspay") . "</p>";
		}
	}
